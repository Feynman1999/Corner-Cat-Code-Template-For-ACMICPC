...
{
    int c=ch-'a';
    if(trans[u][c]!=-1&&maxlen[u]+1==maxlen[trans[u][c]]) return trans[u][c]; //顺着Trie树走
...
{
    int ch[maxn+5][11];//next指针，next指针和字典树类似，指向的串为当前串两端加上同一个字符构成
    ...
    void dfs(int k,ll num,int len)
    {
        for(int i=0;i<=10;++i)
            if(ch[k][i])
            {
                ans=(ans+(num*10%mod+i+i*pw[len+1]%mod)%mod)%mod;
                dfs(ch[k][i],(num*10%mod+i+i*pw[len+1]%mod)%mod,len+2);
            }
    }
    void work()
    {
