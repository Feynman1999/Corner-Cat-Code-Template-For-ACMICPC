...
$$f_n =\sum_{i=0}^n{n\choose i}g_i \\ g_n =\sum_{i=0}^n(-1)^{n-i}{n\choose i}f_i$$

$$f_k=\sum_{i=k}^n{i\choose k}g_i\\
g_k=\sum_{i=k}^n{i\choose k}f_i(-1)^{i-k}$$
...
- $m^n=\sum_{i=0}^m S(n,i){m\choose i}i!=\sum_{i=0}^m S(n,i)m^{\underline i} $
